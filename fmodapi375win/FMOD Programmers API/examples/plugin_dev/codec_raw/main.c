...
FMOD_RESULT F_CALLBACK rawopen(FMOD_CODEC *codec, FMOD_MODE usermode, FMOD_CREATESOUNDEXINFO *userexinfo);
FMOD_RESULT F_CALLBACK rawclose(FMOD_CODEC *codec);
FMOD_RESULT F_CALLBACK rawread(FMOD_CODEC *codec, void *buffer, unsigned int size, unsigned int *read);
FMOD_RESULT F_CALLBACK rawsetposition(FMOD_CODEC *codec, int subsound, unsigned int position, FMOD_TIMEUNIT postype);
...
{          
    ...
    codec->numsubsounds = 0;                    /* in - number of 'subsounds' in this sound.  For most codecs this is 0, only multi sound codecs such as FSB or CDDA have subsounds. */
    codec->waveformat   = &rawwaveformat;
    codec->userdata     = 0;                    /* in - user data value */
