...
namespace FMOD
{
    /* 
        DSP callbacks
    */
    public delegate RESULT DSP_CREATECALLBACK         (ref DSP dsp);
    public delegate RESULT DSP_RELEASECALLBACK        (ref DSP dsp);
    public delegate RESULT DSP_RESETCALLBACK          (ref DSP dsp);
    public delegate RESULT DSP_READCALLBACK           (ref DSP dsp, IntPtr inbuffer, IntPtr outbuffer, uint length, int inchannels, int outchannels);
    public delegate RESULT DSP_SETPOSITIONCALLBACK    (ref DSP dsp, uint seeklen);
    public delegate RESULT DSP_SETPARAMCALLBACK       (ref DSP dsp, int index, float val);
    public delegate RESULT DSP_GETPARAMCALLBACK       (ref DSP dsp, int index, ref float val, StringBuilder valuestr);
    public delegate RESULT DSP_DIALOGCALLBACK         (ref DSP dsp, IntPtr hwnd, bool show);
    ...
    */
    public enum DSP_TYPE
    {
        ...
        LOWPASS2,           /* This unit filters data using an alternate resonant lowpass filter algorithm. */
    ...
    */
    public struct DSP_DESCRIPTION
    {
        ...
        DSP_CREATECALLBACK                 create;             /* [in] Create callback.  This is called when DSP unit is created.  Can be null. */
        DSP_RELEASECALLBACK                release;            /* [in] Release callback.  This is called just before the unit is freed so the user can do any cleanup needed for the unit.  Can be null. */
        DSP_RESETCALLBACK                  reset;              /* [in] Reset callback.  This is called by the user to reset any history buffers that may need resetting for a filter, when it is to be used or re-used for the first time to its initial clean state.  Use to avoid clicks or artifacts. */
        DSP_READCALLBACK                   read;               /* [in] Read callback.  Processing is done here.  Can be null. */
        DSP_SETPOSITIONCALLBACK            setposition;        /* [in] Setposition callback.  This is called if the unit wants to update its position info but not process data.  Can be null. */

        public int                         numparameters;      /* [in] Number of parameters used in this filter.  The user finds this with DSP::getNumParameters */
        DSP_PARAMETERDESC[]                paramdesc;          /* [in] Variable number of parameter structures. */
        DSP_SETPARAMCALLBACK               setparameter;       /* [in] This is called when the user calls DSP::setParameter.  Can be null. */
        DSP_GETPARAMCALLBACK               getparameter;       /* [in] This is called when the user calls DSP::getParameter.  Can be null. */
        DSP_DIALOGCALLBACK                 config;             /* [in] This is called when the user calls DSP::showConfigDialog.  Can be used to display a dialog to configure the filter.  Can be null. */
    ...
    /*
        ...
        Use FMOD_DSP_SetParameter / FMOD::DSP::setParameter with these enums for the 'index' parameter.
    ...
    */
    public enum DSP_ECHO
    {
        ...
        FEEDBACK,    /* 0 = Unit is a simple 1 line delay.  1 = Unit echos what has already been processed by echo. Default = 1. */
