// DRGObject.h - Created with DR&E AppWizard 1.0B9
// ------------------------------------------------------------------------------------
...
HRESULT CDRGObject::SetupIndexedVertexBuffer()
{
	...
	//test (2lines)
	DWORD t = D3DFVF_VERTEX;
	DWORD t2 = 	D3DFVF_XYZ | D3DFVF_NORMAL  | D3DFVF_TEX2;


    vbIndexedDesc.dwFVF         = m_dwVertexFlags;//D3DFVF_VERTEX;
...
/*
		//WARNING! CHEEZY HACK!
		int n = 0;
		if (m_dwVertexFlags & D3DFVF_XYZ)
			n+=3;
		if (m_dwVertexFlags & D3DFVF_NORMAL)
			n+=3;
		if (m_dwVertexFlags & D3DFVF_TEX1)
			n+=2;
		if (m_dwVertexFlags & D3DFVF_TEX2)
			n+=4;
*/
		memcpy(pTemp,m_pVertices,m_iNumVertices * sizeof(D3DVERTEX));
		//memcpy(pTemp,m_pVertices,m_iNumVertices * n * sizeof(float));
...
//			OutputDebugString(" - shader activated\n");
		...
		if (bRenderChildren)
		{
			...
			while (pTempObject != NULL)
			{
				...
				// then we have to reapply the parent's shader
				if ((pTempObject->m_pShader) && (pTempObject->m_pNext) &&
					(pTempObject->m_pNext->m_pShader == NULL) )
				{
					m_pWrapper->m_pD3DDevice7->ApplyStateBlock(pTempObject->m_pShader->m_dwStateBlock);
					//just for debugging			
//					OutputDebugString(pTempShader->m_strShaderName);
