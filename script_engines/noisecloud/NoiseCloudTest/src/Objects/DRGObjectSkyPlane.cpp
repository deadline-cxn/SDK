// DRGObjectSkyPlane.cpp - Created with DR&E AppWizard 1.0B9
// ------------------------------------------------------------------------------------
...
//  PURPOSE:
//    
// DRGObjectSkyPlane.cpp: definition of hte the Plane Object, derived from the DRGObject base class.
...
// -----------------------------------------------------------------------
CDRGObjectSkyPlane::CDRGObjectSkyPlane()
{
	...
	m_fPlanetRadius = 0.0f;
	m_fAtmosphereRadius = 0.0f;
	m_fHTileFactor = 1.0f;
	m_fVTileFactor = 1.0f;
...
HRESULT CDRGObjectSkyPlane::Init(	CDRGWrapper *pDRGWrapper,
					int			divisions,
					/*float		tilefactor,*/
					float		planetradius,
					...
					DWORD		dwFlags			 )
{
	...
	if (divisions > 256) 
		idiv = 256; //64k verts is the max size for a VB 

	/*m_fTileFactor = tilefactor;*/


	m_fPlanetRadius = planetradius;
	m_fAtmosphereRadius = atmosphereradius;
	m_fHTileFactor = h_tilefactor;
	m_fVTileFactor = v_tilefactor;
...
HRESULT CDRGObjectSkyPlane::SetupVertices( )
{
	...
	float planesize = 2.0f * (float)sqrt( (m_fAtmosphereRadius*m_fAtmosphereRadius) - 
										  (m_fPlanetRadius*m_fPlanetRadius) );

	float fDelta = planesize/(float)iNumDivisions;
	float fTexDelta = 2.0f/(float)iNumDivisions;
	int count = 0;
	...
	for (int i=0;i<=iNumDivisions;i++)
	{
		...
		{
			float xdist = (-0.5f * planesize) + ((float)j*fDelta);
			float zdist = (-0.5f * planesize) + ((float)i*fDelta);
			
			float xheight = (xdist*xdist) / m_fAtmosphereRadius;
			float zheight = (zdist*zdist) / m_fAtmosphereRadius;
			float height = xheight + zheight; 
			//m_pVertices[count].x = (-0.5f * planesize) + ((float)j*fDelta); 
			m_pVertices[count].x = xdist; 
			
			m_pVertices[count].y =  0.0f - height; 
			//m_pVertices[count].z = (-0.5f * planesize) + ((float)i*fDelta);
			m_pVertices[count].z = zdist;

			m_pVertices[count].nx = 0.0f; 
			m_pVertices[count].ny = -1.0f; //assume pointing downward
			m_pVertices[count].nz = 0.0f;

			m_pVertices[count].tu = m_fHTileFactor*((float)j * fTexDelta*0.5f);
			m_pVertices[count].tv = m_fVTileFactor*(1.0f - (float)i * fTexDelta*0.5f); 
...
HRESULT CDRGObjectSkyPlane::SetupIndices( )
{
	int	iNumDivisions = (int)sqrt(m_iNumIndices/6);
	int index = 0;
