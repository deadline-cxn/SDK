// DRGObjectPlane.cpp - Created with DR&E AppWizard 1.0B9
// ------------------------------------------------------------------------------------
...
HRESULT CDRGObjectPlane::SetupVertices( )
{
	...
	for (int i=0;i<=iNumDivisions;i++)
	{
		for (int j=0;j<=iNumDivisions;j++)
		{
			...
			m_pVertices[count].tu = (float)j * fDelta * 0.5f * m_fTileFactor;
			m_pVertices[count].tv = m_fTileFactor - (float)i * fDelta * 0.5f * m_fTileFactor; 
