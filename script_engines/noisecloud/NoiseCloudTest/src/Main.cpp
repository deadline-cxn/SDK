// Main.cpp - Created with DR&E AppWizard 1.0B9
// ------------------------------------------------------------------------------------
...
CNoiseCloudTest *g_pNoiseCloudTest;             // the app class where you stick your app-specific stuff
...
long FAR PASCAL WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message) 
	{
		...
		case WM_KEYDOWN:			
			switch(wParam) 
			{
				...
				case 'F':	// Toggle between full screen and windowed
                    if( g_bActive && g_pDRGWrapper && g_pNoiseCloudTest )
					{
						g_bActive = false;
						...
						{
                            hr = g_pNoiseCloudTest->TargetChanged();
			...
			g_pNoiseCloudTest->KeyDown(wParam);
			break;		
		case WM_KEYUP:
			//In this file, we aren't using any key releases, but we'll call 
			// the KeyUp in the app class in case the user needs it
			g_pNoiseCloudTest->KeyUp(wParam);
			break;
		//We're not using the mouse here, but we'll pass it to the app
		case WM_MOUSEMOVE:
			if( (g_pDRGWrapper) && (g_pDRGWrapper->Initialized()) )
				g_pNoiseCloudTest->MouseMove(wParam,lParam);
			break;

		case WM_LBUTTONDOWN:
			if( (g_pDRGWrapper) && (g_pDRGWrapper->Initialized()) )
				g_pNoiseCloudTest->MouseLeftDown(wParam,lParam);
			break;
		
		case WM_LBUTTONUP:
			if( (g_pDRGWrapper) && (g_pDRGWrapper->Initialized()) )
				g_pNoiseCloudTest->MouseLeftUp(wParam,lParam);
			break;

		case WM_RBUTTONDOWN:
			if( (g_pDRGWrapper) && (g_pDRGWrapper->Initialized()) )
				g_pNoiseCloudTest->MouseRightDown(wParam,lParam);
			break;
		
		case WM_RBUTTONUP:
			if( (g_pDRGWrapper) && (g_pDRGWrapper->Initialized()) )
				g_pNoiseCloudTest->MouseRightUp(wParam,lParam);
			break;
		...
		case WM_SIZE:
            ...
            if( g_bActive && g_pDRGWrapper && g_pNoiseCloudTest && !g_bFullscreen )
			{
				g_bActive = false;
		...
		case WM_DESTROY:
			...
			//
            if( g_pNoiseCloudTest ) 
			{
                g_pNoiseCloudTest->DestroyWorld();
                delete g_pNoiseCloudTest;
                g_pNoiseCloudTest = NULL;
			}
...
int FAR PASCAL WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpCmdLine, int nCmdShow)
{
	...
	//
    g_pNoiseCloudTest = new CNoiseCloudTest;
    g_pNoiseCloudTest->InitWorld( g_pDRGWrapper );
	...
	g_pCTitleText->CreateString("Procedural Cloud Demo", 0.0f, 0.02f, 0.04f, 0.06f, 50, &titlestringarray[0]);
	g_pCTitleText->CreateString("(Press H for Help)", 0.0f, 0.8f, 0.02f, 0.03f, 50, &titlestringarray[1]);

    float fYPos = 0.05f;
	i = 0;

	g_pCHelpText->CreateString("P - Pause Animation",            0.0f, fYPos, 0.02f, 0.03f, 50, &helpstringarray[i++]); fYPos += 0.05f;
	g_pCHelpText->CreateString("W - Toggle Wireframe",			 0.0f, fYPos, 0.02f, 0.03f, 50, &helpstringarray[i++]); fYPos += 0.05f;
	g_pCHelpText->CreateString("Z - Toggle XYZ Axis",			 0.0f, fYPos, 0.02f, 0.03f, 50, &helpstringarray[i++]); fYPos += 0.05f;
	g_pCHelpText->CreateString("A - Toggle Time Acceleration",   0.0f, fYPos, 0.02f, 0.03f, 50, &helpstringarray[i++]); fYPos += 0.05f;
	g_pCHelpText->CreateString("R/T - Increase/Decrease Cloud Cover",0.0f, fYPos, 0.02f, 0.03f, 50, &helpstringarray[i++]); fYPos += 0.05f;
	g_pCHelpText->CreateString("S - Vary 0 to 2 Smoothing Steps",0.0f, fYPos, 0.02f, 0.03f, 50, &helpstringarray[i++]); fYPos += 0.05f;
	g_pCHelpText->CreateString("D - Toggle Darkened Clouds",     0.0f, fYPos, 0.02f, 0.03f, 50, &helpstringarray[i++]); fYPos += 0.05f;
	g_pCHelpText->CreateString("1-4 - View Noise Octaves",       0.0f, fYPos, 0.02f, 0.03f, 50, &helpstringarray[i++]); fYPos += 0.05f;
	g_pCHelpText->CreateString("5 - View Composite Noise",       0.0f, fYPos, 0.02f, 0.03f, 50, &helpstringarray[i++]); fYPos += 0.05f;
	g_pCHelpText->CreateString("6 - View Composite Noise Squared", 0.0f, fYPos, 0.02f, 0.03f, 50, &helpstringarray[i++]); fYPos += 0.05f;
	g_pCHelpText->CreateString("7 - View factor Minus Noise",	 0.0f, fYPos, 0.02f, 0.03f, 50, &helpstringarray[i++]); fYPos += 0.05f;
	g_pCHelpText->CreateString("X - Exit",                       0.0f, fYPos, 0.02f, 0.03f, 50, &helpstringarray[i++]); fYPos += 0.05f;
    g_pCHelpText->CreateString("F - Toggle Fullscreen",          0.0f, fYPos, 0.02f, 0.03f, 50, &helpstringarray[i++]); fYPos += 0.05f;
	g_pCHelpText->CreateString("H - Toggle Help",                0.0f, fYPos, 0.02f, 0.03f, 50, &helpstringarray[i++]); fYPos += 0.05f;
	...
	{ 
        ...
        if( g_bActive && g_pNoiseCloudTest && g_pDRGWrapper )
		{
			if( g_pDRGWrapper->Initialized() )
			{
				...
				if (!g_bPauseAnimation)
					g_pNoiseCloudTest->UpdateWorld(); //Step function to update timers, move objects, etc, the right amount

				g_pDRGWrapper->StartFrame(); 

                g_pNoiseCloudTest->RenderWorld(); //Application calls drawprim, d3drm render, or whatever.
