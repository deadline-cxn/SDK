// DRGTexture.cpp - Created with DR&E AppWizard 1.0B9
// ------------------------------------------------------------------------------------
									...
									DWORD dwBpp, DWORD dwTextureStage, DWORD dwTextureFlags, DWORD	*pBits)
{
	...
	{
		DWORD *pTempBits = new DWORD[(4* dwHres * dwVres)];
		ZeroMemory( pTempBits, sizeof(DWORD)*(4* dwHres * dwVres));
		if (SetBitmapBits( hbmBitmap, (4* dwHres * dwVres), pTempBits) == 0)
			return E_FAIL;
		delete pTempBits;
