// DRGText.cpp - Created with DR&E AppWizard 1.0B9
// ------------------------------------------------------------------------------------
...
HRESULT CDRGText::CreateString(char *string, float x, float y, float hsize, float vsize, DWORD color, int *piStringID)
{
	...
	for (int i = 0; i< pCurrString->pNext->iStringLen; i++) 
	{
		...
		for (int j=0; j<6;j++)
		{
			pCurrString->pNext->pVerts[(i*6)+j].sz = 0.005f ; //set Z to the front.
...
HRESULT CDRGText::ModifyStringContent(int iStringID, char *string)
{
	...
	else
	{
		...
		for (int i = 0; i< pCurrString->iStringLen; i++) 
		{
			...
			for (int j=0; j<6;j++)
			{
				pCurrString->pVerts[(i*6)+j].sz = 0.005f ; //set Z to the front.
...
HRESULT CDRGText::DrawText()
{
	...
	else
	{
		...
		r = pDRGWrapper->m_pD3DDevice7->SetTextureStageState( 0,D3DTSS_ALPHAOP, D3DTOP_SELECTARG1); //disable other stages

		r = pDRGWrapper->m_pD3DDevice7->SetTexture(1,NULL);
		r = pDRGWrapper->m_pD3DDevice7->SetTextureStageState(1,D3DTSS_COLOROP, D3DTOP_DISABLE); //disable other stages
		r = pDRGWrapper->m_pD3DDevice7->SetTextureStageState(1,D3DTSS_ALPHAOP, D3DTOP_DISABLE); //disable other stages
		
		
		r = pDRGWrapper->m_pD3DDevice7->SetRenderState( D3DRENDERSTATE_ALPHABLENDENABLE,   TRUE );
		r = pDRGWrapper->m_pD3DDevice7->SetRenderState( D3DRENDERSTATE_SRCBLEND,  D3DBLEND_SRCALPHA );
		r = pDRGWrapper->m_pD3DDevice7->SetRenderState( D3DRENDERSTATE_DESTBLEND, D3DBLEND_INVSRCALPHA );
	...
	r = pDRGWrapper->m_pD3DDevice7->SetTexture(   pDRGTexture->dwStage,  pOldTexture);
